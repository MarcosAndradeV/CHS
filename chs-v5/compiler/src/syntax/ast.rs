...
pub enum EnumVariant {
    ...
    TuplePayload(Token, Vec<Type>),      // Some(T)
    StructPayload(Token, Vec<Field>),    // Circle { radius: int }
}
...
impl EnumVariant {
    pub fn token(&self) -> &Token {
        match self {
            ...
            EnumVariant::TuplePayload(tok, _) => tok,
            EnumVariant::StructPayload(tok, _) => tok,
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct VarTypeValue {
    pub name: Token,
    pub typ: Type,
    pub value: Option<Expr>,
}

pub type FunctionParameter = VarTypeValue;
pub type Field = VarTypeValue;
...
pub struct Expr {
    pub kind: ExprKind,
    pub resolved_type: Option<crate::types::TypeID>,
}
...
pub fn token_to_precedence(token: &Token) -> Precedence {
    match token.kind {
        ...
        TokenKind::Dollar => Precedence::Member,
...
pub fn map_type(ast_ty: &Type, db: &mut t::TypeDatabase) -> t::TypeID {
    fn map_type_name(token: &Token, db: &mut t::TypeDatabase) -> t::TypeID {
        let name = token.source();
        if let Some(id) = db.lookup_by_name(name) {
            id
        } else {
            // Forward declare if referenced before definition
    ...
    match ast_ty {
        Type::Tuple(elements, _) => {
            ...
            for elem in elements {
                mapped_elems.push(map_type(elem, db));
            }
            db.tuple(mapped_elems)
        }
        Type::Pointer(count, inner_ast) => {
            let mut inner = map_type(inner_ast, db);
            for _ in 0..*count {
                inner = db.pointer(inner);
            }
            inner
        }
        Type::Scalar(token) => map_type_name(token, db),
        Type::Array(inner_ast, size) => {
            let inner_ty = map_type(inner_ast, db);
        ...
        Type::GenericInst(base, args) => {
            ...
            for arg in args {
                arg_ids.push(map_type(arg, db));
            }
            let inst_name = mangle_instantiation_name(base_name, &arg_ids, db);
            let id = if let Some(id) = db.lookup_by_name(&inst_name) {
                id
            } else {
                let is_enum = if let Some(base_id) = db.lookup_by_name(base_name) {
        ...
        Type::FnPointer { parameters, return_type, .. } => {
            let mut param_ids = Vec::new();
            for param in parameters {
                param_ids.push(map_type(&param.typ, db));
            }
            let ret_id = match return_type {
                Some(ret) => map_type(ret, db),
                None => db.void(),
            };
