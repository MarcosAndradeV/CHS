use crate::ir::{Function, InstId, Instruction, Module, Operand, Type as TypeID};
use crate::types::{Type as LangType, TypeDatabase};
...
impl StructLayout {
    pub fn compute(type_id: crate::types::TypeID, type_db: &TypeDatabase) -> Self {
...
pub fn type_layout(type_id: crate::types::TypeID, type_db: &TypeDatabase) -> (u32, u32) {
    let canonical = type_db.resolve(type_id);
...
fn collect_string_literals(module: &Module) -> HashMap<std::rc::Rc<str>, usize> {
    ...
    let mut queue: Vec<crate::types::TypeID> =
        module.type_db.queried_types.iter().copied().collect();
    while let Some(ty) = queue.pop() {
        ...
        if visited.insert(canonical) {
            ...
            match module.type_db.get_type(canonical) {
                ...
                crate::types::Type::Array(inner, _) => {
                    queue.push(*inner);
                }
                crate::types::Type::Slice(inner) => {
                    queue.push(*inner);
                }
                ...
                crate::types::Type::FnPointer { params, return_type } => {
                    for param in params {
                        queue.push(*param);
                    }
...
impl<'a> QbeTranspiler<'a> {
    ...
    fn emit_types(&mut self) {
        ...
        fn visit<'b>(
            ...
            temp_visited: &mut HashSet<crate::types::TypeID>,
            order: &mut Vec<crate::types::TypeID>,
        ) {
    ...
    fn map_abi_type(&self, ty: crate::types::TypeID) -> String {
        let canonical = self.module.type_db.resolve(ty);
    ...
    fn map_extended_type(&self, ty: crate::types::TypeID) -> String {
        let canonical = self.module.type_db.resolve(ty);
        if canonical == self.module.type_db.float() {
            "d".to_string()
        } else if canonical == self.module.type_db.void()
    ...
    fn map_base_type(&self, ty: crate::types::TypeID) -> String {
        let canonical = self.module.type_db.resolve(ty);
        if canonical == self.module.type_db.float() {
            "d".to_string()
        } else if canonical == self.module.type_db.void() {
    ...
    fn emit_instruction(&mut self, func: &Function, id: &InstId, inst: &Instruction, ty: &TypeID) {
        ...
        match inst {
            Instruction::Add(l, r) => {
                ...
                let l_is_ptr = matches!(self.module.type_db.get_type(l_canon), LangType::Pointer(_));
                let r_is_ptr = matches!(self.module.type_db.get_type(r_canon), LangType::Pointer(_));

                if l_is_ptr {
                    let LangType::Pointer(inner_ty) = self.module.type_db.get_type(l_canon) else { unreachable!() };
                    let (raw_size, _) = type_layout(*inner_ty, &self.module.type_db);
                    let elem_size = std::cmp::max(raw_size, 1);
                    
                    let ext_inst = if r_canon == self.module.type_db.int() { "extsw" } else { "extuw" };
                    writeln!(self.output, "    %idx_l_{} =l {} {}", id.0, ext_inst, self.op(r)).unwrap();
                    writeln!(self.output, "    %offset_{} =l mul %idx_l_{}, {}", id.0, id.0, elem_size).unwrap();
                    writeln!(self.output, "    %r_{} =l add {}, %offset_{}", id.0, self.op(l), id.0).unwrap();
                } else if r_is_ptr {
                    let LangType::Pointer(inner_ty) = self.module.type_db.get_type(r_canon) else { unreachable!() };
                    let (raw_size, _) = type_layout(*inner_ty, &self.module.type_db);
                    let elem_size = std::cmp::max(raw_size, 1);
                    
                    let ext_inst = if l_canon == self.module.type_db.int() { "extsw" } else { "extuw" };
                    writeln!(self.output, "    %idx_l_{} =l {} {}", id.0, ext_inst, self.op(l)).unwrap();
                    writeln!(self.output, "    %offset_{} =l mul %idx_l_{}, {}", id.0, id.0, elem_size).unwrap();
                    writeln!(self.output, "    %r_{} =l add {}, %offset_{}", id.0, self.op(r), id.0).unwrap();
                } else {
            ...
            Instruction::Sub(l, r) => {
                ...
                if l_is_ptr && r_is_ptr {
                    ...
                    if dest_base == "w" {
                        writeln!(self.output, "    %diff_l_{} =l sub {}, {}", id.0, self.op(l), self.op(r)).unwrap();
                        writeln!(self.output, "    %div_l_{} =l div %diff_l_{}, {}", id.0, id.0, elem_size).unwrap();
                        writeln!(self.output, "    %r_{} =w copy %div_l_{}", id.0, id.0).unwrap();
                    } else {
                        writeln!(self.output, "    %diff_{} =l sub {}, {}", id.0, self.op(l), self.op(r)).unwrap();
                        writeln!(self.output, "    %r_{} =l div %diff_{}, {}", id.0, id.0, elem_size).unwrap();
                    }
                } else if l_is_ptr {
                    let LangType::Pointer(inner_ty) = self.module.type_db.get_type(l_canon) else { unreachable!() };
                    let (raw_size, _) = type_layout(*inner_ty, &self.module.type_db);
                    ...
                    writeln!(self.output, "    %idx_l_{} =l {} {}", id.0, ext_inst, self.op(r)).unwrap();
                    writeln!(self.output, "    %offset_{} =l mul %idx_l_{}, {}", id.0, id.0, elem_size).unwrap();
                    writeln!(self.output, "    %r_{} =l sub {}, %offset_{}", id.0, self.op(l), id.0).unwrap();
                } else {
                    let op_ty = self.get_operand_type(l, func);
                    let op_base_ty = self.map_base_type(op_ty);
                    writeln!(
            ...
            | Instruction::GtEq(comp_ty, l, r) => {
                ...
                if let LangType::Enum { repr, .. } = self.module.type_db.get_type(canonical_comp) {
                    ...
                    let load_inst = if repr_canon == self.module.type_db.int() {
                        "loadw"
                    } else if repr_canon == self.module.type_db.u8() || repr_canon == self.module.type_db.bool() {
                        "loadub"
                    } else {
                        "loadw"
                    };
                    
                    writeln!(self.output, "    %tag_l_{} =w {} {}", id.0, load_inst, self.op(l)).unwrap();
                    writeln!(self.output, "    %tag_r_{} =w {} {}", id.0, load_inst, self.op(r)).unwrap();
                    ...
                    let comp_suffix = if is_float {
                        ...
                        "w".to_string()
                    } else if matches!(self.module.type_db.get_type(canonical_comp), LangType::Pointer(_) | LangType::FnPointer { .. }) {
                        "l".to_string()
                    } else {
                        "w".to_string()
                    };
                    ...
                    let op_name = match inst {
                        ...
                        Instruction::Lt(..) => if is_float { format!("clt{}", comp_suffix) } else { format!("cslt{}", comp_suffix) },
                        Instruction::LtEq(..) => if is_float { format!("cle{}", comp_suffix) } else { format!("csle{}", comp_suffix) },
                        Instruction::Gt(..) => if is_float { format!("cgt{}", comp_suffix) } else { format!("csgt{}", comp_suffix) },
                        Instruction::GtEq(..) => if is_float { format!("cge{}", comp_suffix) } else { format!("csge{}", comp_suffix) },
                        _ => unreachable!(),
                    };
            ...
            Instruction::Cast(op) => {
                ...
                if !is_src_agg && is_dest_agg {
                    ...
                    if let LangType::Enum { repr, .. } = self.module.type_db.get_type(canonical_dest) {
                        let repr_canon = self.module.type_db.resolve(*repr);
                    ...
                    let src_val = if is_src_agg {
                        ...
                        let load_inst = if let LangType::Enum { repr, .. } = self.module.type_db.get_type(canonical_src) {
                            let repr_canon = self.module.type_db.resolve(*repr);
                        ...
                        writeln!(self.output, "    {} =w {} {}", temp_reg, load_inst, self.op(op)).unwrap();
                        temp_reg
                    } else {
                        self.op(op)
                    };

                    let src_base = if is_src_agg { "w".to_string() } else { self.map_base_type(src_ty) };
                    ...
                    if src_base == dest_base {
                        ...
                        let canonical_src = self.module.type_db.resolve(src_ty);
                        let repr_is_int = if let LangType::Enum { repr, .. } = self.module.type_db.get_type(canonical_src) {
                            self.module.type_db.resolve(*repr) == self.module.type_db.int()
                        } else {
                            canonical_src == self.module.type_db.int()
                        };
                        let ext_inst = if repr_is_int {
                            "extsw"
                        } else {
                            "extuw"
                        };
            ...
            Instruction::Load(ptr) => {
                ...
                if self.is_aggregate(val_ty) {
                    ...
                    {
                        "loadub"
                    } else if matches!(self.module.type_db.get_type(underlying_val), LangType::Pointer(_) | LangType::FnPointer { .. })
                    {
                        "loadl"
            ...
            Instruction::Call(callee, args) => {
                let is_indirect = !matches!(callee, Operand::String(_));
                ...
                let (return_type, symbol_name) = if is_indirect {
                    ...
                    if let LangType::FnPointer { return_type, .. } = self.module.type_db.get_type(canonical).clone() {
                    ...
                    (callee_func.signature().return_type, format!("${mangle}{}", callee_func.symbol_name()))
                };
    ...
    fn get_operand_type(&self, op: &Operand, func: &Function) -> crate::types::TypeID {
        match op {
            ...
            Operand::Global(_) => {
                let type_info_id = self.module.type_db.type_info();
                self.module.type_db.pointer_type(type_info_id).unwrap()
            }
        }
    }

    fn mangle_type(&self, ty: crate::types::TypeID) -> String {
        let canonical = self.module.type_db.resolve(ty);
        if canonical == self.module.type_db.void() {
            ...
            match self.module.type_db.get_type(canonical) {
                ...
                LangType::Array(inner, size) => {
                    format!("array_{}_{}", size, self.mangle_type(*inner))
                }
                LangType::Slice(inner) => {
                    format!("slice_{}", self.mangle_type(*inner))
                }
                LangType::Struct { name, .. } => name.clone(),
                LangType::Enum { name, .. } => name.clone(),
                LangType::Tuple(elems) => {
                    let elem_names: Vec<String> =
                        elems.iter().map(|&e| self.mangle_type(e)).collect();
                    format!("chs_tuple_{}", elem_names.join("_"))
                }
                LangType::Distinct { name, .. } => name.clone(),
                LangType::FnPointer { params, return_type } => {
                    let param_names: Vec<String> =
                        params.iter().map(|&p| self.mangle_type(p)).collect();
                    format!("fn_{}_ret_{}", param_names.join("_"), self.mangle_type(*return_type))
                }
                _ => "unknown".to_string(),
            }
        }
    ...
    fn is_aggregate(&self, ty: crate::types::TypeID) -> bool {
        let underlying = self.module.type_db.get_underlying_type(ty);
        matches!(
            self.module.type_db.get_type(underlying),
            LangType::Struct { .. }
                | LangType::Enum { .. }
                | LangType::Array(..)
                | LangType::Slice(..)
    ...
    fn emit_reflection_data(&mut self) {
        let mut visited = std::collections::HashSet::new();
        let mut queue: Vec<crate::types::TypeID> =
            self.module.type_db.queried_types.iter().copied().collect();
        ...
        for &ty in &reflection_types {
            let canonical = self.module.type_db.resolve(ty);
            fn get_type_kind(db: &crate::types::TypeDatabase, ty: crate::types::TypeID) -> u32 {
                let canonical = db.resolve(ty);
                match db.types.get(&canonical).unwrap() {
                    crate::types::Type::Primitive(_) => 1,
                    crate::types::Type::Pointer(_) => 2,
                    crate::types::Type::Array(_, _) => 3,
                    crate::types::Type::Slice(_) => 4,
                    crate::types::Type::Struct { .. } => 5,
                    crate::types::Type::Enum { .. } => 6,
                    crate::types::Type::String(_) => 7,
                    crate::types::Type::Any(_) => 8,
                    crate::types::Type::FnPointer { .. } => 9,
                    crate::types::Type::Distinct { base, .. } => get_type_kind(db, *base),
                    _ => 0,
                }
            }
            ...
            let elem_ptr = match self.module.type_db.get_type(canonical) {
                ...
                crate::types::Type::FnPointer { return_type, .. } => {
                    format!("l $chs_type_info_{}", self.module.type_db.resolve(*return_type).0)
                }
                _ => "l 0".to_string(),
            };

            let arr_len = match self.module.type_db.get_type(canonical) {
                crate::types::Type::Array(_, size) => *size as u64,
                _ => 0,
            };
            ...
            let variants_slice = match self.module.type_db.get_type(canonical) {
                crate::types::Type::Enum { variants, .. } if !variants.is_empty() => {
