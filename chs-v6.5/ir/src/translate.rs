...
use crate::{EnumLayout, StructLayout, ConstVal};
...
pub fn translate_ast_items(
    ...
    let module_paths_ref = &module_paths;
    let get_module_name = |fp: &Path| -> Option<String> {
        module_paths_ref.get(fp).cloned()
    };

    let get_namespaced_name = |name: &str, loc: Loc| -> String {
        let fp = Path::new(*loc.file_path());
        if let Some(m) = module_paths_ref.get(fp) {
            format!("{}.{}", m, name)
        } else {
            name.to_string()
        }
    ...
    for item in items {
        if let s::FileItem::FunctionDecl(decl) = item {
            ...
            for param in &decl.signature.parameters {
                params.push(s::map_type(&param.typ, &mut type_db, &get_module_name));
            }
            let return_type = match &decl.signature.return_type {
                Some(ty) => s::map_type(ty, &mut type_db, &get_module_name),
                None => type_db.void(),
            };
            ...
            let signature = Signature {
                ...
                params,
                return_type,
                mangled_name: decl.resolved_name.clone().unwrap(),
                is_private,
            };
    ...
    for item in items {
        match item {
            s::FileItem::VarDecl(decl) => {
                let init_val = eval_const_expr(&decl.expr);
                let var_ty = if let Some(ref ast_ty) = decl.var_type {
                    s::map_type(ast_ty, &mut module.type_db, &get_module_name)
                } else {
                    match init_val {
                        ConstVal::Int(_) => module.type_db.int(),
                        ConstVal::Bool(_) => module.type_db.bool(),
                        ConstVal::Zero => module.type_db.void(),
                    }
                };
                for name_tok in &decl.names {
                    let namespaced_name = get_namespaced_name(name_tok.source(), name_tok.loc);
        ...
        match item {
            s::FileItem::FunctionDecl(decl) => {
                ...
                if let Some(signature) = env.functions.get(&name).cloned() {
                    let func =
                        translate_function(decl, signature, &env, &mut module.type_db, &module_paths, &file_imports, &module.globals, reporter);
...
fn eval_const_expr(expr: &s::Expr) -> ConstVal {
    match &expr.kind {
        ...
        s::ExprKind::Unary(unary_expr) if unary_expr.op == s::Op::Neg => {
            match eval_const_expr(&unary_expr.right) {
                ConstVal::Int(val) => ConstVal::Int(-val),
                _ => ConstVal::Zero,
            }
        }
        _ => ConstVal::Zero,
...
struct Translator<'a, 'b> {
    ...
    module_paths: &'b HashMap<PathBuf, String>,
    file_imports: &'b HashMap<PathBuf, HashMap<String, PathBuf>>,
    globals: &'b [crate::Global],
...
impl<'a, 'b> Translator<'a, 'b> {
    ...
    fn map_ast_type(&mut self, ast_ty: &s::Type) -> Type {
        ...
        let get_module_name = |fp: &Path| -> Option<String> {
            module_paths.get(fp).cloned()
        };
        s::map_type(ast_ty, self.builder.type_db, &get_module_name)
    }
    ...
    fn lookup_var(&self, name: &str) -> Option<(Operand, Type)> {
        ...
        let namespaced = if let Some(ref fp) = self.current_file {
            if let Some(m) = self.module_paths.get(fp) {
                Some(format!("{}.{}", m, name))
            } else {
                None
            }
        ...
        for g in self.globals {
            let matches_namespaced = namespaced.as_ref().map_or(false, |ns| ns == &g.name);
            if g.name == name || matches_namespaced {
    ...
    fn get_member_offset(&mut self, base_ty: Type, prop_name: &str) -> (u32, Type) {
        ...
        match self.builder.type_db.get_type(canonical) {
            ...
            t::Type::Any(inner) => self.get_member_offset(*inner, prop_name),
            t::Type::String(inner) => self.get_member_offset(*inner, prop_name),
    ...
    fn translate_default_value(&mut self, ty: Type) -> (Operand, Type) {
        ...
        match self.builder.type_db.get_type(canonical).clone() {
            t::Type::Primitive(_) => {
                if canonical == self.builder.type_db.float() {
            ...
            t::Type::Array(elem_ty, size) => {
                ...
                for i in 0..size {
                    ...
                    let offset = (i as u32 * elem_size) as u32;
    ...
    fn translate_lvalue(&mut self, expr: &s::Expr) -> (Operand, Type) {
        match &expr.kind {
            ...
            s::ExprKind::Member(mem) => {
                ...
                } else if let t::Type::Array(elem_ty, _) =
                    self.builder.type_db.get_type(canonical).clone()
                {
                    if prop_name == "data" {
                                    ...
                                    self.builder.type_db.get_type(canonical_base).clone()
                                {
                                    ...
                                    if let Some(payload_ty) = v.payload {
                                            ...
                                            .clone()
                                        {
                                            t::Type::Tuple(elements) => {
                                                for (idx, arg) in
                                                    call.positional_arguments.iter().enumerate()
                                                {
    ...
    fn translate_expr(&mut self, expr: &s::Expr) -> (Operand, Type) {
        let (val, ty) = match &expr.kind {
            ...
            s::ExprKind::Identifier(ident) => {
                ...
                let mut resolved_sig = self.env.functions.get(name);
                if resolved_sig.is_none() {
                    resolved_sig = self.env.functions.get(name);
                }
                if resolved_sig.is_none() {
                    if let Some(ref fp) = self.current_file {
                        if let Some(imports) = self.file_imports.get(fp) {
                            for canonical_path in imports.values() {
                                if let Some(m) = self.module_paths.get(canonical_path) {
                                    let namespaced = format!("{}.{}", m, name);
                                    if let Some(sig) = self.env.functions.get(&namespaced) {
                                        resolved_sig = Some(sig);
                                        break;
                                    }
                                }
                            }
                ...
                if let Some((ptr, ty)) = self.lookup_var(name) {
                    (self.builder.build_load(ty, ptr), ty)
                } else if let Some(sig) = resolved_sig {
...
fn translate_function(
    ...
    {
        let mut translator = Translator::new(blocks, instructions, env, type_db, module_paths, file_imports, globals, reporter);
