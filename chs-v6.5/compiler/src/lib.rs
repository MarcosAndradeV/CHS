...
impl CompilerProcess {
    ...
    fn run_pipeline(
        ...
        let (mut merged_ast_items, exported_decls) =
            self.parse_and_resolve(compiled_paths, linked_objects, &mut module_paths, &mut file_imports, reporter)?;

        // Phase 2: Middle-end (Type Checking & IR Lowering)
        let mut module = self.analyze_and_lower(&mut merged_ast_items, module_paths, file_imports, reporter)?;
    ...
    fn parse_and_resolve(
        ...
        compiled_paths: &mut HashSet<PathBuf>,
        linked_objects: &mut Vec<PathBuf>,
        module_paths: &mut HashMap<PathBuf, String>,
        file_imports: &mut HashMap<PathBuf, HashMap<String, PathBuf>>,
        reporter: &mut DiagnosticReporter,
            ...
            for item in ast.items {
                if let ast::FileItem::Import(import_decl) = item {
                    ...
                    if let Some(r) = resolved {
                        ...
                        let alias = path_parts.last().unwrap().to_string();
                        file_imports.entry(fp.clone()).or_default().insert(alias.clone(), r.clone());
                        ...
                        if r.is_dir() {
                                ...
                                if let Ok(entries) = fs::read_dir(&r) {
                                    for entry in entries.flatten() {
                                        if entry.path().extension().and_then(|s| s.to_str())
                                            == Some("chs")
                                        {
                                            let child_path = fs::canonicalize(&entry.path()).unwrap_or(entry.path());
                                            module_paths.insert(child_path.clone(), alias.clone());
                                            file_queue.push(child_path);
                                        }
                                    }
                                }
                            ...
                            module_paths.insert(r.clone(), alias.clone());
                            if !compiled_paths.contains(&r) {
                                file_queue.push(r);
                            }
                        }
    ...
    fn analyze_and_lower(
        ...
        let mut type_checker = semantics::TypeChecker::new(
            ...
            module_paths,
            file_imports,
            self.foreign_libraries.keys().cloned().collect(),
        );
        ...
        let mut module =
            ir::translate::translate_ast_items(
                merged_ast_items,
                type_checker.type_db,
                type_checker.module_paths,
                type_checker.file_imports,
                reporter
            )?;
