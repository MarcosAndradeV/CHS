...
pub struct TypeChecker<'a> {
    ...
    pub function_templates: HashMap<String, s::FunctionDecl>,
    ...
    resolve_recursive_visited: std::collections::HashSet<TypeID>,
}
...
impl<'a> TypeChecker<'a> {
    pub fn new(reporter: &'a mut DiagnosticReporter) -> Self {
        ...
        Self {
            ...
            resolve_recursive_visited: std::collections::HashSet::new(),
        }
    }
    ...
    fn pass1_gather_signatures(&mut self, items: &mut [s::FileItem]) {
            ...
            match item {
                s::FileItem::Struct(decl) => {
                    ...
                    for field in &decl.fields {
                        let mut field_ty = map_type(&field.typ, &mut self.type_db);
        ...
        for item in items.iter() {
            if let s::FileItem::TypeDecl(td) = item {
                ...
                let mut base_id = map_type(&td.base_type, &mut self.type_db);
        ...
        for item in items.iter_mut() {
            if let s::FileItem::FunctionDecl(decl) = item {
                ...
                if decl.generic_params.is_some() {
                    if self.function_templates.insert(name, decl.clone()).is_some() {
                        self.reporter.report(
                            ...
                            "Overloading of templates are not supported",
                        );
                    }
                ...
                for p in &decl.signature.parameters {
                    let mut param_ty = map_type(&p.typ, &mut self.type_db);
                ...
                let return_type = decl
                    ...
                    .as_ref()
                    .map(|t| {
                        let mut ret_ty = map_type(t, &mut self.type_db);
                ...
                s::FileItem::FunctionDecl(decl) => {
                    ...
                    if let Some(body) = &mut decl.body {
                        ...
                        for param in &decl.signature.parameters {
                            let mut ty = map_type(&param.typ, &mut self.type_db);
        ...
        match stmt {
            ...
            s::Stmt::VarDecl(decl) => {
                ...
                let declared_ty = decl
                    ...
                    .as_ref()
                    .map(|t| {
                        let mut ty = map_type(t, &mut self.type_db);
                        ty = self.resolve_recursive(ty);
                        ty
                    });
                    ...
                    let final_ty = if let Some(decl_ty) = declared_ty {
                            ...
                            && self.type_db.unify(decl_ty, expr_ty).is_err()
                        {
                            if let Some(decl_ty) = self.coerce(expr_ty, decl_ty) {
                                ...
                                self.check_local_array_to_slice_coercion(decl_ty, expr_ty, &decl.expr, name.loc);
                            } else {
                        ...
                        if let s::ExprKind::Default(loc) = &decl.expr.kind {
                            self.reporter.report(*loc, "Cannot infer type for variable initialized to #default".to_string());
                            self.type_db.void()
                        } else {
                            expr_ty
                        }
        ...
        let ty = match &mut expr.kind {
            ...
            s::ExprKind::Identifier(ident) => {
                if let Some(ty) = self.lookup_var(ident.source()) {
                    ...
                    let ref_file = *ident.loc.file_path();
                    let accessible_overloads: Vec<&FunctionSignature> = overloads
                        .iter()
                        .filter(|sig| {
                            if let Some(decl) = self.resolved_fns.get(&sig.mangled_name) {
                                let is_private = decl.directives.iter().any(|d| matches!(d, s::FunctionDirective::Private));
                                if is_private {
                    ...
                    if !accessible_overloads.is_empty() {
                        if accessible_overloads.len() == 1 {
                            let sig = accessible_overloads[0];
                    ...
                    match (
                        ...
                        _ => {
                            if let Some(resolved_sig) =
                                self.try_resolve_signature(bin.op.get_name(), &[left_ty, right_ty], &[], Some(bin.right.loc()))
                            {
            ...
            s::ExprKind::Call(call) => {
                ...
                if explicit_type_args.is_none()
                    && let Some(overloads) = self.symtab.functions.get(&callee_name)
                {
                    for sig in overloads {
                        let decl = match self.resolved_fns.get(&sig.mangled_name) {
                            Some(d) => d,
                            None => continue,
                        };
                ...
                if !concrete_overload_exists && self.function_templates.contains_key(&callee_name) {
                         ...
                         -> Option<Vec<s::Type>> {
                            ...
                            if let Some(ref explicit_args) = explicit_type_args {
                                for (i, param) in generic_params.iter().enumerate() {
                                    let mut ty_id = map_type(&explicit_args[i], &mut self.type_db);
                                ...
                                for (i, p) in parameters.iter().enumerate() {
                                    if p.name.source() == name.source() {
                                        ...
                                        let expected_param_ty =
                                            map_type_with_substs(&p.typ, &infer_map, &mut self.type_db);
                    ...
                    if let Some(concrete_args) = try_match_template(
                ...
                let resolved_sig = self.try_resolve_signature(&callee_name, &positional_arg_types, &named_args, Some(call.loc));

                if let Some(sig) = resolved_sig {
                    call.resolved_name = Some(sig.mangled_name.clone());
                    let signature = self.resolved_fns.get(&sig.mangled_name).unwrap().signature.clone();
                    ...
                    // Match positional arguments next
                    let mut pos_idx = 0;
                    for (i, _p) in signature.parameters.iter().enumerate() {
                        if final_args[i].is_none() {
                            if pos_idx < call.positional_arguments.len() {
                    ...
                    for i in 0..sig.params.len() {
                        if final_args[i].is_none() {
                            let mut cloned_expr = signature.parameters[i].value.as_ref().unwrap().clone();
                    ...
                    let mut final_pos_args: Vec<s::Expr> = final_args.into_iter().map(|o| o.unwrap()).collect();
                    if sig.has_va_args {
                    ...
                    if let Some(left_ty) = self.lookup_var(ident.source()) {
                        if self.type_db.unify(left_ty, right_ty).is_err() {
                            if let Some(left_ty) = self.coerce(left_ty, right_ty) {
                                ...
                                self.check_local_array_to_slice_coercion(left_ty, right_ty, &assign.right, assign.right.loc());
                            } else {
                                let expected_str = self.type_db.type_to_string(left_ty);
                                let found_str = self.type_db.type_to_string(right_ty);
                                let err = SemanticError::TypeMismatch {
            ...
            s::ExprKind::Cast(target_type, expr, loc) => {
                ...
                let mut dest_ty = map_type(target_type, &mut self.type_db);
                                ...
                                if let Some((init_ty, loc)) = init_fields.remove(&def_field.name) {
                                            ...
                                            .find(|fd| fd.name.source() == def_field.name)
                                    {
                                        ...
                                        let mut cloned_expr = match &field_decl.value {
                                            ...
                                            None => s::Expr::new(s::ExprKind::Default(field_decl.name.loc)),
                                        };
                                        let default_ty = self.infer_expr(&mut cloned_expr);
            ...
            s::ExprKind::Array(arr) => {
                if let Some(hint) = &arr.type_hint {
                    let mut hint_ty = map_type(hint, &mut self.type_db);
            ...
            s::ExprKind::TypeInfo(ast_ty, _) => {
                let mut target_ty = map_type(ast_ty, &mut self.type_db);
    ...
    fn monomorphize_struct_with_ids(&mut self, name: &str, arg_ids: &[TypeID]) -> TypeID {
        ...
        for field in &decl.fields {
            let mut mapped_ty = map_type_with_substs(&field.typ, &substs, &mut self.type_db);
    ...
    fn monomorphize_function(
        ...
        for p in &decl.signature.parameters {
            ...
            let mut mapped_ty = map_type(&subst_ty, &mut self.type_db);
            mapped_ty = self.resolve_recursive(mapped_ty);
        ...
        if let Some(ref mut b) = body {
            self.push_scope();
            for p in &parameters {
                let mut ty = map_type(&p.typ, &mut self.type_db);
                ty = self.resolve_recursive(ty);
    ...
    fn check_local_array_to_slice_coercion(&mut self, expected: TypeID, actual: TypeID, expr: &s::Expr, loc: Loc) {
