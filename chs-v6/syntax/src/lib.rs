    ...
    match ptoken.kind {
        TokenKind::Identifier => {
            ...
            match peek.kind {
                TokenKind::OpenParen => {
                    let loc = peek.loc;
                    let (positional_arguments, named_arguments) = try_parse!(lex, parse_arguments(lex, reporter));
                    Parser::Success(
                        lex,
                        Stmt::Call(CallExpr {
                            loc,
...
fn parse_arguments<'lex>(
    ...
    loop {
        ...
        if first_named {
            ...
            let _ = lex.next();
            let arg = try_parse!(lex, parse_named_argument(lex, reporter).or_else(|lex| {
...
fn parse_function_parameter<'lex>(
    ...
    Parser::Success(lex, FunctionParameter { name, typ, value })
}
...
fn parse_type<'lex>(
    ...
    match token.kind {
        ...
        TokenKind::Ampersand | TokenKind::DoubleAmpersand => {
            ...
            let mut pointer_count = if kind == TokenKind::DoubleAmpersand { 2 } else { 1 };
            while lex.peek().kind == TokenKind::Ampersand || lex.peek().kind == TokenKind::DoubleAmpersand {
                let peeked_kind = lex.next().kind;
                pointer_count += if peeked_kind == TokenKind::DoubleAmpersand { 2 } else { 1 };
            }
    ...
    match ptoken.kind {
        ...
        TokenKind::Directive => {
            ...
            if token.source() == "#type_info" && lex.peek().kind == TokenKind::OpenParen {
                ...
                return Parser::Success(
                    lex,
                    Expr::new(ExprKind::Default(token.loc)),
                );
...
fn parse_infix<'lex>(
    ...
    match token.kind {
        ...
        TokenKind::OpenParen => {
            let (lex, (positional_arguments, named_arguments)) = try_parse!(parse_arguments(lex, reporter));
            Parser::Success(
                lex,
