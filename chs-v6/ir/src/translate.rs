...
use crate::{EnumLayout, StructLayout};
    ...
    for item in items {
        if let s::FileItem::FunctionDecl(decl) = item {
            ...
            for param in &decl.signature.parameters {
                params.push(s::map_type(&param.typ, &mut type_db));
            }
            let return_type = match &decl.signature.return_type {
                Some(ty) => s::map_type(ty, &mut type_db),
                None => type_db.void(),
            };
        ...
        match stmt {
            ...
            s::Stmt::VarDecl(decl) => {
                ...
                let declared_ty = decl
                    .var_type
                    .as_ref()
                    .map(|t| s::map_type(t, self.builder.type_db));
    ...
    fn translate_expr(&mut self, expr: &s::Expr) -> (Operand, Type) {
        let (val, ty) = match &expr.kind {
            ...
            s::ExprKind::TypeInfo(ast_ty, _) => {
                let target_ty = s::map_type(ast_ty, self.builder.type_db);
